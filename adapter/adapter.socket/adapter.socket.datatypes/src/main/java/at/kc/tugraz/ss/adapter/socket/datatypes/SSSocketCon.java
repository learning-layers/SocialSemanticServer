...
public class SSSocketCon{
  ...
  public SSSocketCon(
    final String  host, 
    final Integer port, 
    final String  jsonRequ) throws Exception{
    
    this.sSJsonRequ  = jsonRequ;
...
//    ret.put(SSJSONLDU.context, SSJSONLDU.jsonLDContext());
  ...
  public void writeErrorFullToClient(List<SSErrForClient> errors, SSMethU op) throws Exception{
...
//    }
  ...
  public boolean writeFileChunkToClient(byte[] chunk, int chunkLength) throws Exception{
    ...
    if(SSObjU.isNull(chunk)){
      SSServErrReg.regErrThrow(new Exception("chunk to send to client is null"));
    }
  ...
  public boolean writeFileChunkToSS(byte[] chunk) throws Exception{
    ...
    if(SSObjU.isNull(chunk)){
      SSServErrReg.regErrThrow(new Exception("chunk to send to SSS is null"));
    }
  ...
  public void writeRequFullToSS() throws IOException{
    
    sSOutRequFull.write(sSJsonRequ);
    sSOutRequFull.write(SSSocketU.endOfRequest);
    sSOutRequFull.flush();
  }
  
  private void writeMsgFullToClient(String stringJSON) throws Exception{
  ...
  public void closeCon(){
    ...
    try{
      ...
      SSServErrReg.regErr(error, "server data input stream close failed");
    }
  }
}
