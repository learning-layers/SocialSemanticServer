...
public class SSOCDTester extends Thread {
  
  @Override
  public void run() {
    
    final SSOCDConf ocdConf = (SSOCDConf) SSOCDServ.inst.conf;
    
    if(!ocdConf.executeOpAtStartUp){
      return;
    }
    
    switch(ocdConf.op) {
      case ocdCreateGraph : new Thread(new SSOCDGraphsTest (ocdConf)).start(); break;
    }
  }
